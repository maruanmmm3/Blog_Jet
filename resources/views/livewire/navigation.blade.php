<nav class="bg-gray-800" x-data="{ open: false}">
    <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        ...
        <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
          ...
          <div class="hidden sm:block sm:ml-6">
            <div class="flex space-x-4">
              ...
              <a href="{{ route('posts.category', $category) }}" {{-- Nombre de la ruta ---  Variable que lleva --}} class="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">{{ $category->name }}</a>
              @endforeach
          ...
          <div class="ml-3 relative" x-data="{ open: false}">
            ...
            <div>
              <button x-on:click="open = true" class="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu" aria-haspopup="true">
                ...
                <img class="h-8 w-8 rounded-full" src="{{ auth()->user()->profile_photo_url }}" alt=""> {{-- LA FOTO DEL USER AUTENTIFICADO --}}
              </button>
            </div>
            ...
            <div x-show="open" x-on:click.away="open = false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
              ...
              @can('admin.home'){{-- Solo personas con este permiso --}}
                <a href="{{ route('admin.home') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Dashboard</a>
              @endcan

              @can('admin.posts.create'){{-- Solo personas con este permiso --}}
              <a href="{{ route('admin.posts.create') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Crear Un Post</a>
              @endcan
      ...
      @else {{-- DE LO CONRARIO NO ESTA AUTENTIFICADO --}}
